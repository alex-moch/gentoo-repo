...
# overlay. uvicorn's [standard] extra is likewise left unexpanded, matching
# how this ebuild has always handled it.
#
# Four atoms below are loosened from lfx's real upstream pin because the
...
#   - rich:      real pin is <14.0.0, tree only has 15.0.0+
#   - structlog: real pin is <26.0.0, tree only has 26.1.0+
#   - chardet:   real pin is >=7.3.0, tree only has 6.0.0_p1 (older, not newer)
# setuptools' real pin (>=83.0.0,<84.0.0) is a moving target across bumps --
# it happened to line up with what's packaged this time, unlike the two
# prior lfx releases.
RDEPEND="
	>=dev-python/a2wsgi-1.10.0[${PYTHON_USEDEP}]
	...
	>=www-servers/gunicorn-22.0[${PYTHON_USEDEP}]
	...
	>=dev-python/langflow-sdk-0.3.0[${PYTHON_USEDEP}]
	...
	<dev-python/openpyxl-4[${PYTHON_USEDEP}]

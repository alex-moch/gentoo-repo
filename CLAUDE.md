...
| `app-misc/lmstudio-bin` | local | Written from scratch — no upstream ebuild reference (closed source; no releases feed). Reference boilerplate came from three *unofficial* personal overlays (`gkowal/acrux-overlay`, `Deftera186/deftera-overlay`, `kirirururu/gentoo-overlay`) that each already carry an ebuild — none is authoritative and each has real bugs, so treat them as starting points only, not ground truth. Repackages the official `.deb` (~650MB, a prebuilt Electron bundle bundling its own CPython 3.11, node/deno/esbuild, and multiple llama.cpp inference backends) into `/opt/LM-Studio`. `RDEPEND` cross-checked between the `.deb`'s own `control` file `Depends:` line and a full-tree `scanelf -qn -R` (most real ELF deps are in the bundled backend `.so`s, not the main binary) — `x11-libs/libXtst`, `x11-libs/libXScrnSaver`, and `sys-apps/util-linux` were all in the `.deb`'s `Depends:` but have zero references anywhere in the extracted tree, so were dropped (electron-builder boilerplate cruft, same class of finding as the `claude-desktop` 1.32352.1 bump). `media-libs/vulkan-loader` and `media-gfx/vips` are NOT dependencies despite the app linking `libvulkan.so.1`/`libvips-cpp.so` — both are fully bundled (confirmed: the Vulkan loader itself ships inside the archive in three places, and the `sharp` Node addon carries its own `libvips-cpp.so` next to itself). `IUSE="cuda vulkan"` gates the optional bundled CUDA (~600MB) and Vulkan (~21MB) llama.cpp backends; there is no ROCm backend in this build despite some reference overlays declaring `IUSE=rocm` for it. Arm64 has no `.deb` at all (AppImage-only, confirmed via the download page's own JSON), so this is amd64-only for now. Version checks need `.tools/check-bumps/overrides.json` — no feed exists; scrape `https://lmstudio.ai/download`'s embedded JSON (`"linux":{"x64":{"version","build"}}`) and verify the resulting download URL actually 200s before trusting it |
| `app-text/zotero-bin` | Gentoo (official) | Forked, maintainer reassigned, version-bumped ahead of the official tree (9.0.6 → 10.0 — the official tree hadn't packaged it as of the fork). Watch out: upstream's git tag for this release is `10.0.0`, but the only download version string that actually resolves on `download.zotero.org` is `10.0` — `10.0.0` returns an S3 `AccessDenied` on every platform (no live artifact), confirmed before trusting the bump. `RDEPEND` re-derived from a full `scanelf -qn -R` across the whole bundle (most of the real dependency surface lives in the bundled `libxul.so`, not `zotero-bin` itself) plus a `strings`/`grep` sweep for dlopen'd sonames inside it: `dev-libs/dbus-glib` dropped — zero references anywhere in the bundle, Gecko's D-Bus integration now goes through a bundled Rust `dbus` crate linking `libdbus-1` directly (confirmed via `third_party/rust/dbus/src/*.rs` strings in `libxul.so`). `x11-libs/libXtst` and `app-accessibility/at-spi2-core` are absent from the ELF `NEEDED` set too but kept — both are genuinely dlopen'd (confirmed via string matches for `Xtst`/`atk-bridge` inside `libxul.so`) |
...
| `app-misc/lmstudio-bin` | `https://lmstudio.ai/download` | No releases feed (closed source) and no machine-readable version endpoint — scrape the page's embedded (HTML-escaped) JSON for `\"linux\":{\"x64\":{\"version\":...,\"build\":...}}`. The download URL and the `.deb`'s own `Version:` field both join the two with a separator (`version-build` in the URL, `version+build` inside the `.deb`), so treat them as two independent fields, not one string to bump. Verify with `curl -sI -L "https://installers.lmstudio.ai/linux/x64/<PV>-<BUILD>/LM-Studio-<PV>-<BUILD>-x64.deb"` and confirm a `200` before trusting it. Since it's a binary repackage, re-scan the bundle's ELF `NEEDED` sonames (`scanelf -qn -R` across the whole extracted tree — most real deps are in the bundled llama.cpp backend `.so`s, not the main binary) plus a full-tree `grep` for dlopen'd sonames and check whether the `.deb`'s own `control` `Depends:` line still agrees with what's actually reachable in the tree before trusting the old `RDEPEND` |
| `app-text/zotero-bin` | `https://api.github.com/repos/zotero/zotero/tags` | **Do not trust the tag name as the download version.** Zotero's git tags (e.g. `10.0.0`) don't always match the published download version string (`10.0`) — verify with `curl -sI -L "https://www.zotero.org/download/client/dl?channel=release&platform=linux-x86_64&version=<candidate>"` and confirm a `200` (a tag with no live artifact 302s through to an S3 `AccessDenied` `403`) before trusting a bump. Since it's a binary repackage, re-scan the bundle's ELF `NEEDED` sonames (`scanelf -qn -R` across the whole extracted tree, not just `zotero-bin` — most real deps live in the bundled `libxul.so`) plus a `strings`/`grep` sweep for dlopen'd sonames before trusting the old `RDEPEND` |
